...
## 1. Problem Statement
Given two strings `word1` and `word2`, return the **minimum number of operations** required to convert `word1` to `word2`. Allowed operations (each costs 1): Insert a character, Delete a character, Replace a character.
...
def minDistance_brute(word1, word2):
    def dp(i, j):
        ...
        return 1 + min(dp(i-1, j),    # delete from word1
...
- If characters match: `dp[i][j] = dp[i-1][j-1]` (free operation).
...
- **Space Complexity:** O(m × n) — reducible to O(min(m,n)) with 1D DP
